...
exports.SDKGeneratorAdapter = void 0;
 ...
 */
class SDKGeneratorAdapter extends core_1.Adapter {
    constructor() {
        super();
     ...
     */
    generateTypeScriptSDK(outputDir) {
        return __awaiter(this, void 0, void 0, function* () {
            yield fs.mkdir(outputDir, { recursive: true });
            // Generate main client class
            const clientCode = yield this.generateTypeScriptClient();
            yield fs.writeFile(path.join(outputDir, 'client.ts'), clientCode);
            // Generate type definitions
            const typesCode = yield this.generateTypeScriptTypes();
            yield fs.writeFile(path.join(outputDir, 'types.ts'), typesCode);
            // Generate API methods
            const apiCode = yield this.generateTypeScriptAPI();
            yield fs.writeFile(path.join(outputDir, 'api.ts'), apiCode);
            // Generate examples
            const examplesCode = yield this.generateTypeScriptExamples();
            yield fs.writeFile(path.join(outputDir, 'examples.ts'), examplesCode);
            // Generate package.json
            const packageJson = yield this.generateTypeScriptPackageJson();
            yield fs.writeFile(path.join(outputDir, 'package.json'), packageJson);
            // Generate README
            const readme = yield this.generateTypeScriptReadme();
            yield fs.writeFile(path.join(outputDir, 'README.md'), readme);
        });
    }
    /**
...
`;
     ...
     */
    generatePythonSDK(outputDir) {
        return __awaiter(this, void 0, void 0, function* () {
            yield fs.mkdir(outputDir, { recursive: true });
            // Generate main client
            const clientCode = this.generatePythonClient();
            yield fs.writeFile(path.join(outputDir, 'client.py'), clientCode);
            // Generate setup.py
            const setupPy = this.generatePythonSetup();
            yield fs.writeFile(path.join(outputDir, 'setup.py'), setupPy);
            // Generate requirements.txt
            const requirements = 'requests>=2.28.0\naiohttp>=3.8.0\npydantic>=1.10.0\n';
            yield fs.writeFile(path.join(outputDir, 'requirements.txt'), requirements);
        });
    }
    /**
...
exports.SDKGeneratorAdapter = SDKGeneratorAdapter;
