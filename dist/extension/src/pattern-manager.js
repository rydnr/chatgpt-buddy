...
exports.globalPatternManager = exports.PatternManager = void 0;
...
class PatternManager {
    ...
    async createPattern(name, description, steps, options = {}) {
        ...
        message_store_1.globalMessageStore.addInboundMessage('PATTERN_CREATED', { patternId: pattern.id, name, category: pattern.category }, `pattern-created-${Date.now()}`, { extensionId: chrome.runtime.id, userAgent: navigator.userAgent });
    ...
    async updatePattern(id, updates) {
        ...
        message_store_1.globalMessageStore.addInboundMessage('PATTERN_UPDATED', { patternId: id, version: updatedPattern.version }, `pattern-updated-${Date.now()}`, { extensionId: chrome.runtime.id, userAgent: navigator.userAgent });
    ...
    async deletePattern(id) {
        ...
        message_store_1.globalMessageStore.addInboundMessage('PATTERN_DELETED', { patternId: id, name: pattern.name }, `pattern-deleted-${Date.now()}`, { extensionId: chrome.runtime.id, userAgent: navigator.userAgent });
    ...
    async exportPatterns(patternIds, format = 'json', options = {}) {
        ...
        // Track export
        message_store_1.globalMessageStore.addInboundMessage('PATTERNS_EXPORTED', {
    ...
    async importPatterns(data, options = {}) {
        ...
        try {
            ...
            // Track import
            message_store_1.globalMessageStore.addInboundMessage('PATTERNS_IMPORTED', {
    ...
    async sharePatterns(patternIds, shareType = 'private', options = {}) {
        ...
        await this.saveShares();
        // Track sharing
        message_store_1.globalMessageStore.addInboundMessage('PATTERNS_SHARED', { shareId, patternCount: patternIds.length, shareType }, `patterns-shared-${Date.now()}`, { extensionId: chrome.runtime.id, userAgent: navigator.userAgent });
    ...
    async accessSharedPatterns(shareId) {
        ...
        // Track access
        message_store_1.globalMessageStore.addInboundMessage('SHARED_PATTERNS_ACCESSED', { shareId, patternId: share.patternId }, `shared-patterns-accessed-${Date.now()}`, { extensionId: chrome.runtime.id, userAgent: navigator.userAgent });
        return [pattern];
    }
    /**
...
exports.globalPatternManager = new PatternManager();
