...
function connectWebSocket(serverUrl) {
    ...
    ws.onmessage = async (event) => {
        try {
            ...
            message_store_js_1.globalMessageStore.addInboundMessage(message.type || 'UNKNOWN_MESSAGE_TYPE', message, message.correlationId || `inbound-${Date.now()}`, metadata);
...
class MessageHandler {
    ...
    sendResponse(response, correlationId) {
        try {
            ...
            message_store_js_1.globalMessageStore.addOutboundMessage(response.type || 'RESPONSE', response, correlationId, metadata);
            // Send WebSocket response
            ws?.send(JSON.stringify(response));
            globalThis.extensionTestData.lastResponse = response;
            // Mark the original message as successful
            message_store_js_1.globalMessageStore.markMessageSuccess(correlationId, response);
            console.log('📤 Response sent and tracked:', response);
        }
        catch (error) {
            console.error('❌ Failed to send response:', error);
            message_store_js_1.globalMessageStore.markMessageError(correlationId, error instanceof Error ? error.message : 'Unknown error');
        }
    }
    ...
    sendErrorResponse(correlationId, error, additionalData) {
        ...
        try {
            ...
            message_store_js_1.globalMessageStore.addOutboundMessage('ERROR_RESPONSE', errorResponse, correlationId, metadata);
            ...
            message_store_js_1.globalMessageStore.markMessageError(correlationId, error);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === 'getMessageStoreState') {
        sendResponse({
            success: true,
            state: message_store_js_1.globalMessageStore.getState(),
            statistics: message_store_js_1.globalMessageStore.getState().messages.length > 0 ?
                (() => {
                    const stats = { total: 0, success: 0, error: 0, pending: 0, inbound: 0, outbound: 0 };
                    message_store_js_1.globalMessageStore.getState().messages.forEach(msg => {
                        stats.total++;
                        stats[msg.status]++;
                        stats[msg.direction]++;
                    });
