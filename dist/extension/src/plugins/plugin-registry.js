...
class PluginRegistry {
    ...
    async register(plugin, manifest, context) {
        ...
        try {
            ...
            if (this.plugins.has(pluginId)) {
                throw new plugin_interface_1.PluginLoadError(`Plugin ${pluginId} is already registered`, pluginId);
            }
            // Check dependencies
            ...
            await this.emitPluginEvent(plugin_interface_1.PluginEvents.PLUGIN_LOADED, pluginId, {
            ...
            const pluginError = error instanceof plugin_interface_1.PluginError ? error :
                new plugin_interface_1.PluginLoadError(`Failed to register plugin ${pluginId}: ${error.message}`, pluginId, error);
            await this.emitPluginEvent(plugin_interface_1.PluginEvents.PLUGIN_ERROR, pluginId, {
    ...
    async unregister(pluginId) {
        ...
        if (!registration) {
            throw new plugin_interface_1.PluginError(`Plugin ${pluginId} is not registered`, pluginId, 'PLUGIN_NOT_FOUND');
        }
        try {
            // Check if other plugins depend on this one
            if (registration.dependents.length > 0) {
                throw new plugin_interface_1.PluginDependencyError(`Cannot unregister plugin ${pluginId}: ${registration.dependents.length} plugins depend on it`, pluginId, { dependents: registration.dependents });
            }
            ...
            await this.emitPluginEvent(plugin_interface_1.PluginEvents.PLUGIN_DESTROYED, pluginId);
            console.log(`🗑️ Plugin unregistered: ${pluginId}`);
        }
        catch (error) {
            const pluginError = error instanceof plugin_interface_1.PluginError ? error :
                new plugin_interface_1.PluginError(`Failed to unregister plugin ${pluginId}: ${error.message}`, pluginId, 'UNREGISTER_ERROR');
            await this.emitPluginEvent(plugin_interface_1.PluginEvents.PLUGIN_ERROR, pluginId, {
                error: pluginError.message
            });
            throw pluginError;
    ...
    async initialize(pluginId) {
        ...
        if (registration.state !== 'uninitialized') {
            throw new plugin_interface_1.PluginError(`Plugin ${pluginId} is not in uninitialized state`, pluginId, 'INVALID_STATE');
        }
        try {
            ...
            await this.emitPluginEvent(plugin_interface_1.PluginEvents.PLUGIN_INITIALIZED, pluginId);
            ...
            const pluginError = new plugin_interface_1.PluginInitializationError(`Failed to initialize plugin ${pluginId}: ${error.message}`, pluginId, error);
            await this.emitPluginEvent(plugin_interface_1.PluginEvents.PLUGIN_ERROR, pluginId, {
                error: pluginError.message,
    ...
    async activate(pluginId) {
        ...
        if (registration.state !== 'initialized' && registration.state !== 'inactive') {
            throw new plugin_interface_1.PluginError(`Plugin ${pluginId} cannot be activated from ${registration.state} state`, pluginId, 'INVALID_STATE');
        }
        try {
            ...
            await this.emitPluginEvent(plugin_interface_1.PluginEvents.PLUGIN_ACTIVATED, pluginId);
            ...
            registration.errorCount++;
            registration.lastError = error;
            const pluginError = new plugin_interface_1.PluginError(`Failed to activate plugin ${pluginId}: ${error.message}`, pluginId, 'ACTIVATION_ERROR', error);
            await this.emitPluginEvent(plugin_interface_1.PluginEvents.PLUGIN_ERROR, pluginId, {
                error: pluginError.message,
                phase: 'activation'
            });
    ...
    async deactivate(pluginId) {
        ...
        try {
            ...
            await this.emitPluginEvent(plugin_interface_1.PluginEvents.PLUGIN_DEACTIVATED, pluginId);
            ...
            registration.errorCount++;
            registration.lastError = error;
            const pluginError = new plugin_interface_1.PluginError(`Failed to deactivate plugin ${pluginId}: ${error.message}`, pluginId, 'DEACTIVATION_ERROR', error);
            await this.emitPluginEvent(plugin_interface_1.PluginEvents.PLUGIN_ERROR, pluginId, {
                error: pluginError.message,
                phase: 'deactivation'
            });
    ...
    async discoverPlugins(options = {}) {
        ...
        try {
            ...
            throw new plugin_interface_1.PluginError(`Plugin discovery failed: ${error.message}`, 'SYSTEM', 'DISCOVERY_ERROR', error);
        }
    }
    /**
    ...
    async sendMessageToPlugin(pluginId, message) {
        ...
        if (registration.state !== 'active') {
            throw new plugin_interface_1.PluginError(`Plugin ${pluginId} is not active`, pluginId, 'PLUGIN_NOT_ACTIVE');
        }
        try {
            const event = {
                type: 'message',
            ...
            throw new plugin_interface_1.PluginError(`Failed to send message to plugin ${pluginId}: ${error.message}`, pluginId, 'MESSAGE_ERROR', error);
        }
    }
    /**
    ...
    async shutdown() {
        ...
        try {
            ...
            this.plugins.clear();
            this.loadOrder = [];
            await this.emitPluginEvent(plugin_interface_1.PluginEvents.SYSTEM_SHUTDOWN, 'system');
    ...
    validatePlugin(plugin, manifest) {
        if (!plugin.id || typeof plugin.id !== 'string') {
            throw new plugin_interface_1.PluginLoadError('Plugin must have a valid string ID', plugin.id || 'unknown');
        }
        if (!plugin.name || typeof plugin.name !== 'string') {
            throw new plugin_interface_1.PluginLoadError('Plugin must have a valid string name', plugin.id);
        }
        if (!plugin.version || typeof plugin.version !== 'string') {
            throw new plugin_interface_1.PluginLoadError('Plugin must have a valid string version', plugin.id);
        }
        if (!manifest.metadata || manifest.metadata.id !== plugin.id) {
            throw new plugin_interface_1.PluginLoadError('Plugin ID must match manifest metadata ID', plugin.id);
        }
        ...
        for (const method of requiredMethods) {
            if (typeof plugin[method] !== 'function') {
                throw new plugin_interface_1.PluginLoadError(`Plugin must implement ${method} method`, plugin.id);
            }
        }
    }
    async checkDependencies(dependencies) {
        for (const depId of dependencies) {
            const depRegistration = this.plugins.get(depId);
            if (!depRegistration) {
                throw new plugin_interface_1.PluginDependencyError(`Required dependency ${depId} is not available`, depId);
            }
        }
    }
    ...
    setupEventHandlers() {
        ...
        this.eventBus.on(plugin_interface_1.PluginEvents.SYSTEM_SHUTDOWN, async () => {
            await this.shutdown();
        });
    }
