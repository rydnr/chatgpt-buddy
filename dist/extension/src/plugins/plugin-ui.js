...
exports.PluginUIFactory = exports.PluginUIManager = exports.UIEventType = void 0;
...
class PluginUIManager {
    ...
    registerComponent(pluginId, component) {
        try {
            if (this.components.has(component.id)) {
                throw new plugin_interface_1.PluginError(`UI component with id ${component.id} already registered`, pluginId, 'UI_COMPONENT_DUPLICATE');
            }
            const entry = {
                component,
                state: 'unmounted',
    ...
    async mountComponent(componentId, mountPoint) {
        const entry = this.components.get(componentId);
        if (!entry) {
            throw new plugin_interface_1.PluginError(`Cannot mount non-existent component: ${componentId}`, 'unknown', 'UI_COMPONENT_NOT_FOUND');
        }
        if (entry.state === 'mounted') {
        ...
        try {
            ...
            if (!targetMountPoint) {
                throw new plugin_interface_1.PluginError(`No mount point available for component type: ${entry.component.type}`, entry.pluginId, 'UI_MOUNT_POINT_UNAVAILABLE');
            }
            // Render component
            const element = await entry.component.render();
            if (!element) {
                throw new plugin_interface_1.PluginError(`Component render() returned null/undefined: ${componentId}`, entry.pluginId, 'UI_RENDER_FAILED');
            }
    ...
    registerMenuItem(pluginId, menuItem) {
        try {
            if (this.menuItems.has(menuItem.id)) {
                throw new plugin_interface_1.PluginError(`Menu item with id ${menuItem.id} already registered`, pluginId, 'UI_MENU_ITEM_DUPLICATE');
            }
            const entry = {
                item: menuItem,
                pluginId,
...
exports.PluginUIManager = PluginUIManager;
/**
...
exports.PluginUIFactory = PluginUIFactory;
//# sourceMappingURL=plugin-ui.js.map