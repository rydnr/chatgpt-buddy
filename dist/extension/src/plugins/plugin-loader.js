...
class PluginLoader {
    ...
    async loadFromManifest(manifest, options = {}) {
        ...
        try {
            ...
            if (this.loadedPlugins.has(pluginId) && !options.allowReload) {
                throw new plugin_interface_1.PluginLoadError(`Plugin ${pluginId} is already loaded`, pluginId);
            }
            // Validate manifest
            if (!options.skipValidation) {
                const validation = await this.validateManifest(manifest);
                if (!validation.valid) {
                    throw new plugin_interface_1.PluginLoadError(`Plugin validation failed: ${validation.errors.join(', ')}`, pluginId);
                }
            }
            // Check dependencies
            ...
            const loadError = error instanceof plugin_interface_1.PluginError ? error :
                new plugin_interface_1.PluginLoadError(`Failed to load plugin ${pluginId}: ${error.message}`, pluginId, error);
    ...
    async loadFromPackage(pluginPackage, options = {}) {
        try {
            ...
            throw new plugin_interface_1.PluginLoadError(`Failed to load plugin package: ${error.message}`, 'unknown', error);
        }
    }
    /**
    ...
    async loadFromURL(url, options = {}) {
        try {
            ...
            throw new plugin_interface_1.PluginLoadError(`Failed to load plugin from URL ${url}: ${error.message}`, 'unknown', error);
        }
    }
    /**
     * Unload a plugin
    ...
    async unload(pluginId) {
        ...
        if (!loadResult) {
            throw new plugin_interface_1.PluginError(`Plugin ${pluginId} is not loaded`, pluginId, 'PLUGIN_NOT_LOADED');
        }
        try {
            ...
            throw new plugin_interface_1.PluginError(`Failed to unload plugin ${pluginId}: ${error.message}`, pluginId, 'UNLOAD_ERROR');
        }
    }
    /**
     * Get loaded plugin
    ...
    async createPluginInstance(manifest, options) {
        ...
        try {
            ...
            throw new plugin_interface_1.PluginInitializationError(`Failed to create plugin instance for ${pluginId}: ${error.message}`, pluginId, error);
        }
    }
    async loadPluginScript(scriptPath) {
        ...
        throw new plugin_interface_1.PluginLoadError('Plugin script loading not implemented', 'unknown');
    }
    ...
    requireModule(moduleName, sandboxConfig) {
        ...
        if (sandboxConfig && !sandboxConfig.allowedModules.includes(moduleName)) {
            throw new plugin_interface_1.PluginSecurityError(`Module ${moduleName} is not allowed in sandbox`, 'unknown', 'MODULE_NOT_ALLOWED');
        }
    ...
    async checkDependencies(dependencies) {
        for (const depId of dependencies) {
            if (!this.loadedPlugins.has(depId)) {
                throw new plugin_interface_1.PluginDependencyError(`Required dependency ${depId} is not loaded`, depId);
            }
        }
    }
        ...
        if (policy.allowedDomains && policy.allowedDomains.length > 0 && !policy.allowedDomains.includes('*')) {
            ...
            for (const domain of supportedDomains) {
                if (!policy.allowedDomains.some(allowed => domain.match(new RegExp(allowed.replace(/\*/g, '.*'))))) {
                    throw new plugin_interface_1.PluginSecurityError(`Domain ${domain} is not allowed by security policy`, manifest.metadata.id, 'DOMAIN_NOT_ALLOWED');
                }
            }
        }
        ...
        if (policy.allowedPermissions && policy.allowedPermissions.length > 0) {
            ...
            for (const permission of requestedPermissions) {
                if (!policy.allowedPermissions.includes(permission)) {
                    throw new plugin_interface_1.PluginSecurityError(`Permission ${permission} is not allowed by security policy`, manifest.metadata.id, 'PERMISSION_NOT_ALLOWED');
                }
            }
        }
        // Validate API access
        if (policy.allowedAPIs && policy.allowedAPIs.length > 0) {
            ...
            for (const api of requiredAPIs) {
                if (!policy.allowedAPIs.includes(api)) {
                    throw new plugin_interface_1.PluginSecurityError(`API ${api} is not allowed by security policy`, manifest.metadata.id, 'API_NOT_ALLOWED');
                }
            }
        }
    }
...
class PluginValidationError extends plugin_interface_1.PluginError {
    constructor(message, pluginId, details) {
        super(message, pluginId, 'PLUGIN_VALIDATION_ERROR', details);
        this.name = 'PluginValidationError';
    }
}
class PluginExecutionError extends plugin_interface_1.PluginError {
    constructor(message, pluginId, details) {
