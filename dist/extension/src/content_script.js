...
const contract_discovery_adapter_1 = require("./contracts/contract-discovery-adapter");
const contract_execution_service_1 = require("./contracts/contract-execution-service");
...
(async () => {
    try {
        await contract_discovery_adapter_1.contractDiscovery.initialize();
...
async function saveUserInteraction(eventType, target, success, context = {}) {
    try {
        ...
        await storage_1.webBuddyStorage.saveUserInteraction(interaction);
...
async function handleAutomationRequest(message) {
    ...
    try {
        const contractResult = await contract_execution_service_1.contractExecution.executeWithContract({
            action,
            parameters,
            domain: currentDomain
        });
    ...
    const existingPatterns = await storage_1.webBuddyStorage.getAutomationPatterns({
        domain: currentDomain,
        action: action,
        successOnly: true,
        limit: 5
    ...
    try {
        switch (action) {
            case 'getStorageStats':
                const stats = await storage_1.webBuddyStorage.getStorageStats();
                return {
                    correlationId,
                    success: true,
                    stats: stats,
            ...
            case 'getAutomationPatterns':
                const patterns = await storage_1.webBuddyStorage.getAutomationPatterns({
                    domain: currentDomain,
            ...
            case 'clearOldData':
                await storage_1.webBuddyStorage.clearOldData(payload.days || 30);
                return {
                    correlationId,
                    success: true,
...
async function handleContractExecution(message) {
    ...
    try {
        const result = await contract_execution_service_1.contractExecution.executeWithContract({
            action,
            parameters,
            preferredContract,
            timeout,
...
async function handleContractDiscovery(message) {
    ...
    try {
        const contracts = await contract_discovery_adapter_1.contractDiscovery.discoverContracts();
        const availability = await contract_execution_service_1.contractExecution.checkContractAvailability();
        const actions = contract_execution_service_1.contractExecution.getAvailableActions();
        return {
            correlationId,
            status: 'success',
            data: {
...
async function handleContractAvailabilityCheck(message) {
    ...
    try {
        const availability = await contract_execution_service_1.contractExecution.checkContractAvailability();
        const recommendations = contract_execution_service_1.contractExecution.getContractRecommendations();
        return {
            correlationId,
            status: 'success',
            data: {
