...
const mockMessageStore = {
    ...
    exportMessages: globals_1.jest.fn(() => JSON.stringify({ messages: [] })),
...
describe('🔄 Message Store Integration Tests', () => {
    ...
    describe('📨 Message Tracking and Storage', () => {
        test('should track inbound messages from WebSocket', async () => {
            ...
            mockMessageStore.addInboundMessage(inboundMessage.type, inboundMessage, inboundMessage.correlationId, metadata);
            expect(mockMessageStore.addInboundMessage).toHaveBeenCalledWith('AutomationRequested', inboundMessage, 'test-123', metadata);
        });
        test('should track outbound responses with success status', async () => {
            ...
            mockMessageStore.addOutboundMessage('RESPONSE', successResponse, 'test-123', metadata);
            // Simulate marking as successful
            mockMessageStore.markMessageSuccess('test-123', successResponse);
            expect(mockMessageStore.addOutboundMessage).toHaveBeenCalledWith('RESPONSE', successResponse, 'test-123', metadata);
        ...
        test('should track outbound error responses', async () => {
            ...
            mockMessageStore.addOutboundMessage('ERROR_RESPONSE', errorResponse, 'test-456', { extensionId: 'test-extension-id', userAgent: 'test-agent' });
            // Simulate marking as failed
            mockMessageStore.markMessageError('test-456', 'Content script not reachable');
            expect(mockMessageStore.addOutboundMessage).toHaveBeenCalledWith('ERROR_RESPONSE', errorResponse, 'test-456', expect.objectContaining({ extensionId: 'test-extension-id' }));
            expect(mockMessageStore.markMessageError).toHaveBeenCalledWith('test-456', 'Content script not reachable');
        });
    });
    ...
    describe('🔍 Message Filtering and Search', () => {
        test('should support message filtering by type', () => {
            ...
            mockMessageStore.getState.mockReturnValue(mockState);
            const state = mockMessageStore.getState();
            const automationMessages = state.messages.filter(msg => state.filters.types.length === 0 || state.filters.types.includes(msg.type));
        ...
        test('should support message filtering by status', () => {
            ...
            mockMessageStore.getState.mockReturnValue(mockState);
            const state = mockMessageStore.getState();
            const successMessages = state.messages.filter(msg => state.filters.statuses.length === 0 || state.filters.statuses.includes(msg.status));
        ...
        test('should support message filtering by direction', () => {
            ...
            mockMessageStore.getState.mockReturnValue(mockState);
            const state = mockMessageStore.getState();
            const inboundMessages = state.messages.filter(msg => state.filters.directions.length === 0 || state.filters.directions.includes(msg.direction));
    ...
    describe('💾 Import/Export Functionality', () => {
        ...
        test('should import messages from JSON format', () => {
            ...
            const success = mockMessageStore.importMessages(JSON.stringify(importData));
        ...
        test('should handle invalid import data gracefully', () => {
            ...
            const success = mockMessageStore.importMessages('invalid json');
    ...
    describe('🔄 Store Subscription and Updates', () => {
        test('should support store subscriptions', () => {
            const mockListener = globals_1.jest.fn();
            const mockUnsubscribe = globals_1.jest.fn();
            mockMessageStore.subscribe.mockReturnValue(mockUnsubscribe);
            const unsubscribe = mockMessageStore.subscribe(mockListener);
...
//# sourceMappingURL=message-store-integration.test.js.map