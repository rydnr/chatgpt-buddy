...
class SimplePatternManager {
    ...
    async createPattern(name, description, steps) {
        const pattern = {
...
describe('🧩 Pattern Management - Simplified', () => {
    ...
    describe('🎨 Pattern Creation', () => {
        test('should create a valid pattern', async () => {
            const pattern = await patternManager.createPattern('Login Pattern', 'Automates login process', sampleSteps);
        ...
        test('should emit creation event', async () => {
            ...
            const pattern = await patternManager.createPattern('Test Pattern', 'Test description', sampleSteps);
    ...
    describe('📤 Export/Import', () => {
        test('should export patterns to JSON', async () => {
            ...
            // Check that both patterns are present (order may vary)
            const patternNames = parsed.patterns.map((p) => p.name);
            expect(patternNames).toContain('Pattern 1');
            expect(patternNames).toContain('Pattern 2');
        });
...
//# sourceMappingURL=pattern-management-simple.test.js.map