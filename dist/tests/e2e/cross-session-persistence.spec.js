...
test_1.test.describe('💾 Cross-Session Pattern Persistence E2E', () => {
    let context;
    let page;
    // Create shared test page for consistency across sessions
    const createTestPage = async (page) => {
    ...
    (0, test_1.test)('💾 Pattern learning and persistence across application restarts', async () => {
        // PHASE 1: Learn patterns in first session
        let trainingApp = new training_application_1.ChatGPTBuddyTrainingApplication();
        ...
        (0, test_1.expect)(patterns.length).toBe(2);
        ...
        (0, test_1.expect)(projectPattern).toBeDefined();
        (0, test_1.expect)(taskPattern).toBeDefined();
        (0, test_1.expect)(projectPattern.selector).toContain('select-webapp');
        (0, test_1.expect)(taskPattern.selector).toContain('task-name');
        ...
        (0, test_1.expect)(persistedPatterns.length).toBe(2);
    ...
    (0, test_1.test)('🔄 Pattern updates and version management across sessions', async () => {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (result.eventType === 'AutomationPatternExecuted') {
                ...
                await (0, test_1.expect)(page.locator('[data-testid="export-report"]')).toBeVisible();
            }
        ...
        (0, test_1.expect)(updatedPattern).toBeDefined();
        (0, test_1.expect)(updatedPattern.usageCount).toBeGreaterThan(0);
        (0, test_1.expect)(updatedPattern.confidence).toBeGreaterThanOrEqual(1.0);
        await trainingApp.stop();
        ...
        (0, test_1.expect)(finalPattern).toBeDefined();
        (0, test_1.expect)(finalPattern.usageCount).toBeGreaterThan(0);
        await trainingApp.stop();
    });
    (0, test_1.test)('📊 Pattern analytics persistence and aggregation', async () => {
        ...
        (0, test_1.expect)(initialStats.totalPatterns).toBe(3);
        (0, test_1.expect)(initialStats.averageConfidence).toBeCloseTo(1.2, 1);
        (0, test_1.expect)(initialStats.successRate).toBeCloseTo(0.84, 2); // (4+6+11)/(5+8+12)
        await trainingApp.stop();
        ...
        (0, test_1.expect)(updatedStats.totalPatterns).toBe(4);
        (0, test_1.expect)(updatedStats.successRate).toBeCloseTo(0.86, 2); // (4+6+11+3)/(5+8+12+3)
        await trainingApp.stop();
        // PHASE 3: Verify statistical persistence
        trainingApp = new training_application_1.ChatGPTBuddyTrainingApplication();
        await trainingApp.start();
        const persistedStats = await trainingApp.getPatternStatistics();
        (0, test_1.expect)(persistedStats.totalPatterns).toBe(4);
        (0, test_1.expect)(persistedStats.patternsByType['ClickElementRequested']).toBe(2);
        (0, test_1.expect)(persistedStats.patternsByType['FillTextRequested']).toBe(1);
        (0, test_1.expect)(persistedStats.patternsByType['SelectOptionRequested']).toBe(1);
        await trainingApp.stop();
    });
    (0, test_1.test)('🗑️ Pattern cleanup and storage management across sessions', async () => {
        ...
        (0, test_1.expect)(beforeCleanup.totalPatterns).toBe(3);
        // Perform cleanup (30 day threshold)
        const deletedCount = await trainingApp.cleanupStalePatterns(30);
        (0, test_1.expect)(deletedCount).toBeGreaterThan(0); // Should delete the old bad pattern
        const afterCleanup = await trainingApp.getPatternStatistics();
        (0, test_1.expect)(afterCleanup.totalPatterns).toBeLessThan(beforeCleanup.totalPatterns);
        await trainingApp.stop();
        // PHASE 2: Verify cleanup persistence
        trainingApp = new training_application_1.ChatGPTBuddyTrainingApplication();
        await trainingApp.start();
        const persistedAfterCleanup = await trainingApp.getPatternStatistics();
        (0, test_1.expect)(persistedAfterCleanup.totalPatterns).toBe(afterCleanup.totalPatterns);
        ...
        (0, test_1.expect)(badPatternExists).toBe(false); // Bad pattern should be gone
        ...
        (0, test_1.expect)(goodPatternExists || recentPatternExists).toBe(true); // Good patterns should remain
        await trainingApp.stop();
    });
    (0, test_1.test)('🔄 Export/Import pattern data across sessions', async () => {
        // PHASE 1: Create and export patterns
        let trainingApp = new training_application_1.ChatGPTBuddyTrainingApplication();
        await trainingApp.start();
        const exportTestPatterns = [
            {
                id: 'export-pattern-1',
                messageType: 'ClickElementRequested',
        ...
        (0, test_1.expect)(exportedData.length).toBe(2);
        (0, test_1.expect)(exportedData.map(p => p.id)).toContain('export-pattern-1');
        (0, test_1.expect)(exportedData.map(p => p.id)).toContain('export-pattern-2');
        await trainingApp.stop();
        ...
        (0, test_1.expect)(emptyPatterns.length).toBe(2); // Should still have patterns from previous session
        // Clear all patterns
        ...
        (0, test_1.expect)(reimportedPatterns.length).toBe(exportedData.length);
        ...
        (0, test_1.expect)(reimportedPattern1).toBeDefined();
        (0, test_1.expect)(reimportedPattern1.messageType).toBe(originalPattern1.messageType);
        (0, test_1.expect)(reimportedPattern1.selector).toBe(originalPattern1.selector);
        (0, test_1.expect)(reimportedPattern1.confidence).toBe(originalPattern1.confidence);
        await trainingApp.stop();
        // PHASE 3: Verify persistence after import
        trainingApp = new training_application_1.ChatGPTBuddyTrainingApplication();
        await trainingApp.start();
        const finalPersistedPatterns = await trainingApp.exportPatterns();
        (0, test_1.expect)(finalPersistedPatterns.length).toBe(2);
        await trainingApp.stop();
    });
    (0, test_1.test)('⚡ Performance with large pattern datasets across sessions', async () => {
        ...
        (0, test_1.expect)(importTime).toBeLessThan(5000); // Should complete within 5 seconds
        const stats = await trainingApp.getPatternStatistics();
        (0, test_1.expect)(stats.totalPatterns).toBe(50);
        await trainingApp.stop();
        ...
        (0, test_1.expect)(exportTime).toBeLessThan(1000); // Should export within 1 second
        (0, test_1.expect)(allPatterns.length).toBe(50);
        ...
        (0, test_1.expect)(matchingTime).toBeLessThan(500); // Should match within 500ms
        await trainingApp.stop();
    });
    (0, test_1.test)('🛡️ Data integrity and corruption recovery', async () => {
        // PHASE 1: Create patterns with specific data
        let trainingApp = new training_application_1.ChatGPTBuddyTrainingApplication();
        await trainingApp.start();
        const integrityTestPattern = {
        ...
        (0, test_1.expect)(storedPattern).toBeDefined();
        (0, test_1.expect)(storedPattern.payload.metadata.checksum).toBe('abc123');
        (0, test_1.expect)(storedPattern.payload.metadata.version).toBe('1.0.0');
        await trainingApp.stop();
        ...
        (0, test_1.expect)(reloadedPattern).toBeDefined();
        (0, test_1.expect)(reloadedPattern.payload.metadata.checksum).toBe('abc123');
        (0, test_1.expect)(reloadedPattern.payload.metadata.version).toBe('1.0.0');
        // Test that complex nested data structures are preserved
        (0, test_1.expect)(typeof reloadedPattern.payload.metadata).toBe('object');
        (0, test_1.expect)(reloadedPattern.payload.metadata.created).toBeTruthy();
        await trainingApp.stop();
    });
});
...
//# sourceMappingURL=cross-session-persistence.spec.js.map