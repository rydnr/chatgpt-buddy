...
const client_1 = require("../../packages/web-buddy-core/src/client");
...
describe('🌐 Browser Extension E2E Integration', () => {
    ...
    const EXTENSION_PATH = path_1.default.resolve(__dirname, '../../extension');
    beforeAll(async () => {
        ...
        client = new client_1.WebBuddyClient({
     ...
     */
    (0, test_1.test)('🔴 should send message from client through server to browser extension', async () => {
        ...
        (0, test_1.expect)(response.correlationId).toBe(automationEvent.correlationId);
        ...
        const extensionLogs = await page.evaluate(() => {
            return window.extensionTestData?.lastReceivedMessage;
        });
        (0, test_1.expect)(extensionLogs).toBeDefined();
        (0, test_1.expect)(extensionLogs.type).toBe('automationRequested');
        (0, test_1.expect)(extensionLogs.payload.action).toBe('testAction');
    });
    /**
     * 🔴 RED: Test extension can execute DOM actions in response to server commands
     */
    (0, test_1.test)('🔴 should execute DOM actions via extension when commanded by server', async () => {
    ...
    `);
        ...
        (0, test_1.expect)(inputValue).toBe('E2E test input value');
        // Verify the extension reported success back to server
        const lastResponse = await page.evaluate(() => {
            return window.extensionTestData?.lastResponse;
        });
        (0, test_1.expect)(lastResponse).toBeDefined();
        (0, test_1.expect)(lastResponse.status).toBe('success');
        (0, test_1.expect)(lastResponse.correlationId).toBe(fillInputEvent.correlationId);
    });
    /**
     * 🔴 RED: Test real-time WebSocket communication with extension
     */
    (0, test_1.test)('🔴 should maintain persistent WebSocket connection with extension', async () => {
        ...
        (0, test_1.expect)(messageReceived).toBe(true);
        ...
        const wsLogs = await page.evaluate(() => {
            return window.extensionTestData?.webSocketMessages || [];
        });
        (0, test_1.expect)(wsLogs.length).toBeGreaterThan(0);
        (0, test_1.expect)(wsLogs.some((msg) => msg.type === 'ping')).toBe(true);
    });
});
//# sourceMappingURL=browser-extension.e2e.test.js.map