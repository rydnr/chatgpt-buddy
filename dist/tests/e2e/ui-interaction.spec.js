...
test_1.test.describe('🎨 UI Interaction and Element Selection E2E', () => {
    let context;
    let page;
    let trainingApp;
    ...
    (0, test_1.test)('🎨 Training overlay appearance and styling', async () => {
        ...
        await (0, test_1.expect)(overlay).toBeVisible();
        await (0, test_1.expect)(overlay).toHaveCSS('position', 'fixed');
        await (0, test_1.expect)(overlay).toHaveCSS('z-index', '10000');
        // Check overlay content
        await (0, test_1.expect)(overlay).toContainText('Training Mode Active');
        await (0, test_1.expect)(overlay).toContainText('ClickElementRequested');
        await (0, test_1.expect)(overlay).toContainText('create task button');
        ...
        (0, test_1.expect)(bodyCursor).toBe('crosshair');
    });
    (0, test_1.test)('🖱️ Element highlighting during selection mode', async () => {
        ...
        (0, test_1.expect)(inputOutline).toContain('blue'); // Should have blue outline from training system
    ...
    (0, test_1.test)('🎯 Element selection and confirmation flow', async () => {
        ...
        await (0, test_1.expect)(confirmationDialog).toHaveCSS('z-index', '10001'); // Higher than training overlay
        // Check confirmation dialog content
        await (0, test_1.expect)(confirmationDialog).toContainText('Element Selected');
        await (0, test_1.expect)(confirmationDialog).toContainText('save-draft');
        await (0, test_1.expect)(confirmationDialog).toContainText('Do you want to automate');
        // Check available action buttons
        await (0, test_1.expect)(confirmationDialog.locator('button:has-text("Yes, Automate")')).toBeVisible();
        await (0, test_1.expect)(confirmationDialog.locator('button:has-text("Select Different Element")')).toBeVisible();
        await (0, test_1.expect)(confirmationDialog.locator('button:has-text("Cancel")')).toBeVisible();
    });
    (0, test_1.test)('✅ Confirmation dialog actions', async () => {
        ...
        await (0, test_1.expect)(page.locator('.training-confirmation-overlay')).not.toBeVisible();
        // AND: Body cursor should return to normal
        const bodyCursor = await page.evaluate(() => window.getComputedStyle(document.body).cursor);
        (0, test_1.expect)(bodyCursor).not.toBe('crosshair');
        ...
        (0, test_1.expect)(learnedPattern.selector).toContain('task-description');
    });
    (0, test_1.test)('🔄 Select different element workflow', async () => {
        // GIVEN: Training confirmation dialog is showing
        await trainingApp.start();
        await trainingApp.enableTrainingMode('localhost');
        const request = {
        ...
        const confirmationDialog = page.locator('.training-confirmation-overlay');
        await (0, test_1.expect)(confirmationDialog).toContainText('quick-add-task');
    });
    (0, test_1.test)('❌ Training cancellation scenarios', async () => {
        ...
        await (0, test_1.expect)(page.locator('.training-overlay')).not.toBeVisible();
        const patterns = await trainingApp.exportPatterns();
        (0, test_1.expect)(patterns.length).toBe(0);
    ...
    (0, test_1.test)('🎛️ Complex form element selection', async () => {
        ...
        for (const element of formElements.slice(0, 2)) { // Test first 2 elements
            ...
            const confirmationDialog = page.locator('.training-confirmation-overlay');
            await (0, test_1.expect)(confirmationDialog).toContainText(element.selector.replace('[data-testid="', '').replace('"]', ''));
    ...
    (0, test_1.test)('📱 Responsive UI elements selection', async () => {
        ...
        (0, test_1.expect)(overlayBox.width).toBeLessThanOrEqual(375);
        ...
        (0, test_1.expect)(confirmationBox.width).toBeLessThanOrEqual(375);
        await page.locator('button:has-text("Yes, Automate")').click();
    ...
    (0, test_1.test)('🎨 Visual feedback and animations', async () => {
        ...
        for (const selector of testElements) {
            ...
            (0, test_1.expect)(elementStyle.outline || elementStyle.backgroundColor).toBeTruthy();
        }
        ...
        (0, test_1.expect)(dialogBox.x).toBeGreaterThan(0);
        (0, test_1.expect)(dialogBox.y).toBeGreaterThan(0);
        (0, test_1.expect)(dialogBox.width).toBeGreaterThan(200);
        (0, test_1.expect)(dialogBox.height).toBeGreaterThan(100);
    });
});
...
//# sourceMappingURL=ui-interaction.spec.js.map