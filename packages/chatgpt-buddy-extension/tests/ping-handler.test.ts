...
import { ExtensionPingHandler } from '../src/domain/ping.handler';

describe('ExtensionPingHandler', () => {
  let handler: ExtensionPingHandler;

  beforeEach(() => {
    handler = new ExtensionPingHandler();
  });
  ...
  it('should handle ping event and return pong with browser info', async () => {
    ...
    const pingEvent = new PingEvent({ message, correlationId });

    // Act
    const result = await handler.handle(pingEvent);

    // Assert
    expect(result).toBeInstanceOf(PongEvent);
