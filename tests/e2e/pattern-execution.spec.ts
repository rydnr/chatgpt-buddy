...
test.describe('🎯 Pattern Matching and Auto-Execution E2E', () => {
  ...
  test('🚀 High-confidence pattern auto-execution', async () => {
    // GIVEN: High-confidence patterns stored in application
    await trainingApp.start();
    
    const highConfidencePatterns: AutomationPatternData[] = [
      {
    ...
    await expect(page.locator('[data-testid="project-react"]')).toHaveClass(/selected/);
    ...
    expect(inputValue).toBe('Implement user authentication');
  });

  test('🎯 Pattern matching with payload variations', async () => {
    // GIVEN: Patterns that should match similar requests
    await trainingApp.start();
    
    const basePattern: AutomationPatternData = {
    ...
    if (result.eventType === 'AutomationPatternExecuted') {
      await expect(page.locator('[data-testid="project-vue"]')).toHaveClass(/selected/);
    }
    // Pattern matching may or may not succeed depending on similarity threshold
    expect(['AutomationPatternExecuted', 'PatternExecutionFailed', 'ElementSelectionRequested']).toContain(result.eventType);
  });

  test('⚡ Multiple pattern execution sequence', async () => {
    // GIVEN: Multiple sequential patterns
    await trainingApp.start();
    
    const sequentialPatterns: AutomationPatternData[] = [
      {
    ...
    for (const request of requests) {
      const result = await trainingApp.handleAutomationRequest(request as AutomationRequest);
      results.push(result);
    ...
    await expect(page.locator('[data-testid="project-node"]')).toHaveClass(/selected/);
    await expect(page.locator('[data-testid="chat-debugging"]')).toHaveClass(/selected/);
    
    const taskValue = await page.locator('[data-testid="task-input"]').inputValue();
    expect(taskValue).toBe('Fix memory leak in user service');
    
    // Verify save button was clicked (check logs)
    const logContent = await page.locator('#execution-log').textContent();
    expect(logContent).toContain('Task saved successfully');
  });

  test('🔄 Pattern confidence updates after execution', async () => {
    // GIVEN: Pattern with moderate confidence
    await trainingApp.start();
    
    const moderatePattern: AutomationPatternData = {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (result.eventType === 'AutomationPatternExecuted') {
        ...
        const logContent = await page.locator('#execution-log').textContent();
        expect(logContent).toContain('Task submitted for review');
      }
    ...
    if (updatedPattern) {
      expect(updatedPattern.usageCount).toBeGreaterThan(3);
      expect(updatedPattern.confidence).toBeGreaterThanOrEqual(0.8);
    }
  });

  test('❌ Pattern execution failure and recovery', async () => {
    // GIVEN: Pattern with invalid selector
    await trainingApp.start();
    
    const invalidPattern: AutomationPatternData = {
    ...
    if (updatedPattern) {
      expect(updatedPattern.confidence).toBeLessThan(1.0);
    }
  });

  test('🎨 Complex form filling pattern execution', async () => {
    // GIVEN: Comprehensive form filling patterns
    await trainingApp.start();
    
    const formPatterns: AutomationPatternData[] = [
      {
        id: 'task-input-fill',
        messageType: 'FillTextRequested',
        payload: { element: 'task input' },
    ...
    const taskValue = await page.locator('[data-testid="task-input"]').inputValue();
    expect(taskValue).toBe('Implement advanced pattern matching algorithm');

    const descriptionValue = await page.locator('[data-testid="description-textarea"]').inputValue();
    expect(descriptionValue).toContain('sophisticated pattern matching system');
  });

  test('🔍 Pattern similarity matching edge cases', async () => {
    // GIVEN: Patterns with various similarity challenges
    await trainingApp.start();
    
    const edgeCasePatterns: AutomationPatternData[] = [
      {
    ...
    const exactMatch: AutomationRequest = {
      messageType: 'SelectProjectRequested',
      payload: { projectName: 'Angular App' },
      context: getCurrentContext(page)
    };
    ...
    expect(['AutomationPatternExecuted', 'PatternExecutionFailed']).toContain(exactResult.eventType);

    // Test case 2: Case difference
    const caseVariation: AutomationRequest = {
      messageType: 'SelectProjectRequested',
      payload: { projectName: 'angular app' },
      context: getCurrentContext(page)
    };
    ...
    expect(['AutomationPatternExecuted', 'PatternExecutionFailed', 'ElementSelectionRequested']).toContain(caseResult.eventType);

    // Test case 3: Partial match
    const partialMatch: AutomationRequest = {
      messageType: 'SelectProjectRequested',
    ...
    expect(['AutomationPatternExecuted', 'PatternExecutionFailed', 'ElementSelectionRequested']).toContain(partialResult.eventType);

    // Test case 4: Completely different
    const noMatch: AutomationRequest = {
      messageType: 'SelectProjectRequested',
    ...
    expect(noMatchResult.eventType).toBe('PatternExecutionFailed');
  });
});
