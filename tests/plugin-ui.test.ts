...
import { PluginUIManager, UIEventType, PluginUIFactory } from '../extension/src/plugins/plugin-ui';
import { DefaultPluginEventBus } from '../extension/src/plugins/plugin-communication';
import type { PluginUIComponent, PluginMenuItem, PluginEventBus, PluginLogger, PluginEvent } from '../extension/src/plugins/plugin-interface';
...
describe('Plugin UI System', () => {
  ...
  beforeEach(() => {
    ...
    // Create UI manager
    uiManager = new PluginUIManager(eventBus, logger);
    uiFactory = new PluginUIFactory();
  });
  ...
  describe('PluginUIManager', () => {
    ...
    test('should register UI component', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('UI component registered'),
    ...
    test('should mount UI component', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('UI component mounted'),
        expect.objectContaining({ pluginId: 'test-plugin' })
      );
    });
    ...
    test('should unmount UI component', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('UI component unmounted'),
        expect.objectContaining({ pluginId: 'test-plugin' })
      );
    });
    ...
    test('should unregister UI component', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('UI component unregistered'),
        expect.objectContaining({ pluginId: 'test-plugin' })
      );
    });
    ...
    test('should register menu item', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('Menu item registered'),
        expect.objectContaining({ pluginId: 'test-plugin' })
      );
    });
    ...
    test('should unregister menu item', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('Menu item unregistered'),
        expect.objectContaining({ pluginId: 'test-plugin' })
      );
    });
    ...
    test('should handle component mount errors', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('Failed to mount UI component'),
        expect.any(Error),
        expect.objectContaining({ pluginId: 'test-plugin' })
      );
    });
    ...
    test('should emit UI events', async () => {
      ...
      eventBus.on(UIEventType.COMPONENT_REGISTERED, eventSpy);
    ...
    test('should handle menu item clicks', async () => {
      ...
      const clickHandler = menuElement.addEventListener.mock.calls.find(
  ...
  describe('PluginUIFactory', () => {
    test('should create UI manager instance', () => {
      ...
      expect(manager).toBeInstanceOf(PluginUIManager);
    });
  });
  ...
  describe('Component Lifecycle', () => {
    ...
    test('should handle component state transitions', async () => {
      const eventSpy = jest.fn();
      eventBus.on(UIEventType.COMPONENT_MOUNTED, eventSpy);
      eventBus.on(UIEventType.COMPONENT_UNMOUNTED, eventSpy);
  ...
  describe('Menu Item Management', () => {
    test('should handle menu item with submenu', () => {
      const submenuItem: PluginMenuItem = {
        id: 'submenu-item',
  ...
  describe('Error Handling', () => {
    ...
    test('should handle menu item action errors', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('Menu item action failed'),
        expect.any(Error),
        expect.objectContaining({ pluginId: 'test-plugin' })
      );
    });
