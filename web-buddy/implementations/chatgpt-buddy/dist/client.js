...
exports.ChatGPTBuddyClient = void 0;
...
class ChatGPTBuddyClient {
     ...
     */
    async selectProject(projectName, options) {
        return this.webBuddyClient.sendMessage({
            [messages_1.ChatGPTMessages.SELECT_PROJECT]: { projectName }
        }, {
            tabId: options?.tabId
        });
    }
     ...
     */
    async selectChat(chatTitle, chatId, options) {
        return this.webBuddyClient.sendMessage({
            [messages_1.ChatGPTMessages.SELECT_CHAT]: { chatTitle, chatId }
        }, {
            tabId: options?.tabId
        });
    }
     ...
     */
    async submitPrompt(prompt, model, options) {
        return this.webBuddyClient.sendMessage({
            [messages_1.ChatGPTMessages.SUBMIT_PROMPT]: { prompt, model }
        }, {
            tabId: options?.tabId
        });
    }
     ...
     */
    async getResponse(waitForCompletion = true, options) {
        const response = await this.webBuddyClient.sendMessage({
            [messages_1.ChatGPTMessages.GET_RESPONSE]: { waitForCompletion }
        }, {
            tabId: options?.tabId
        });
     ...
     */
    async startNewChat(title, options) {
        return this.webBuddyClient.sendMessage({
            [messages_1.ChatGPTMessages.START_NEW_CHAT]: { title }
        }, {
            tabId: options?.tabId
        });
    }
     ...
     */
    async exportConversation(format = 'markdown', options) {
        const response = await this.webBuddyClient.sendMessage({
            [messages_1.ChatGPTMessages.EXPORT_CONVERSATION]: { format }
        }, {
            tabId: options?.tabId
        });
     ...
     */
    async uploadFile(fileData, fileName, mimeType, options) {
        return this.webBuddyClient.sendMessage({
            [messages_1.ChatGPTMessages.UPLOAD_FILE]: { fileData, fileName, mimeType }
        }, {
            tabId: options?.tabId
        });
    }
     ...
     */
    async askQuestion(projectName, question, options) {
     ...
     */
    async conversation(projectName, prompts, options) {
        await this.selectProject(projectName, options);
        if (options?.chatTitle) {
     ...
     */
    async researchWorkflow(projectName, topic, options) {
     ...
     */
    async generateCode(projectName, specification, language, options) {
